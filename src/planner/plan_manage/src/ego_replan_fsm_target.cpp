...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    {
      waypoint_sub_ = nh.subscribe("waypoint_generator/waypoints", 1, &EGOReplanFSM::waypointCallback_forWP, this);
  ...
  void EGOReplanFSM::waypointCallback_forWP(const nav_msgs::PathConstPtr &msg)
  {
    have_trigger_ = true;
    cout << "Triggered!" << endl;
    init_pt_ = odom_pos_;
  ...
  bool EGOReplanFSM::callReboundReplan(bool flag_use_poly_init, bool flag_randomPolyTraj)
  {
    ...
    {
      // 这里是为了把ego当前端用，之后换成hyper a*
      // 如果是1号飞机就不二次优化了
      bool yaw_plan_and_refine_success = false;

      cout << "yaw_plan_success=" << yaw_plan_and_refine_success << endl;
      ...
      // yaw 的轨迹
      if( yaw_plan_and_refine_success )
      {
        // 这里是生成yaw轨迹后生成的数据
